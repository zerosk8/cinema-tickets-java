...
public class BarzahlungsWerkzeug extends BeobachtbaresSubWerkzeug
{
    ...
    {
        if (eingabeBetrag.istGroesserOderGleichAls(_preis)) // eingabeBetrag >= _preis
        {
    ...
    {
        Geldbetrag differenz = eingabeBetrag.substrahieren(_preis);
        _ui.getRestTextfeld().setText("" + differenz.toString());
        if (eingabeBetrag.istKleinerAls(_preis)) // eingabeBetrag < _preis
        {
    ...
    {
        _ui.getPreisTextfeld().setText("" + _preis.toString());
    }
}
